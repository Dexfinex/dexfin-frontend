...
export function parseNumberCorrectly(val: string): number {
  if (!val) {
    return 0
  }
  return parseFloat(detectedSeparator !== '.' ? val.toString().replace(detectedSeparator, '.') : val)
}
