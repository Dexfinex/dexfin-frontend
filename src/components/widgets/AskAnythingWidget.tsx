...
export const AskAnythingWidget: React.FC = () => {
  ...
  const processCommand = (text: string) => {
    ...
    try {
      const commands = text.split(/\s+(?:and|&)\s+/i);



      commands.forEach(async (command) => {
        const normalizedCommand = command.trim();
        const response = await openaiService.getOpenAIWidgetData(normalizedCommand);
        if (response.type == 'open_swap') {
          setIsSwapOpen(true);
        } else if (response.type == 'open_defi') {
          setIsDefiOpen(true);
        } else if (response.type == 'open_dashboard') {
          setIsDashboardOpen(true);
        } else if (response.type == 'open_market_data') {
          setIsMarketDataOpen(true);
        } else if (response.type == 'open_games') {
          setIsGamesOpen(true);
        } else if (response.type == 'open_show_trending') {
          setIsDashboardOpen(true);
        } else if (response.type == 'open_chat') {
          setIsChatOpen(true);
        } else if (response.type == 'open_cart') {
          setIsCartOpen(true);
        } 
        else if (response.type == 'open_social_feed') {
          setIsSocialFeedOpen(true);
        } else if (response.type == 'open_trade') {
          setTradeOpen(true);
        } else if (response.type == 'open_assistant') {
          setWidgetCommand(normalizedCommand);
  ...
  <div className="relative w-full px-4 sm:px-0">
    ...
    <div className={`glass w-[320px] sm:w-[600px] h-14 bg-black/40 backdrop-blur-xl border border-white/10 rounded-lg flex items-center ${isSmallScreen ? 'px-2' : 'px-4'} gap-2 shadow-lg mx-auto`}>
      ...
      <div className="flex items-center gap-1 sm:gap-2 flex-shrink-0">
        <button
          className={`p-1 sm:p-1.5 rounded-md transition-colors ${isListening
              ? 'bg-red-500/50'
              : theme === 'light'
        ...
        <button
          onClick={handleClick}
          className={`p-1 sm:p-1.5 rounded-md transition-colors ${theme === 'light'
              ? 'hover:bg-black/20 text-gray-900'
              : 'hover:bg-white/10 text-white'
