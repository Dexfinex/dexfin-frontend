...
import { Bell, Plus, Trash2, RefreshCw, Edit } from "lucide-react"
import { alertApiService } from "../../services/alert.service"
import { type Alert, type CreateAlertDto, type UpdateAlertDto, alertTypes } from "../../types/alert.types"
...
export const MarketAlerts: React.FC = () => {
    ...
    const handleEditAlert = async () => {
        ...
        try {
            ...
            setAlerts((prev) => prev.map((alert) => (alert.id === editingAlertId ? updatedAlert : alert)))
    ...
    const getTypeLabel = (type: string) => {
        ...
        if (!config) {
            ...
            return (
                <span className="px-3 py-1 rounded-full text-sm bg-gray-500/20 text-gray-400">{type.replace("_", " ")}</span>
            )
        }
                        ...
                        <div key={field.name}>
                            ...
                            {field.type === "select" ? (
                                field.name === "token" ? (
                                    ...
                                    >
                                        <option className={theme === 'dark' ? "bg-[#1a1a1a]" : "bg-[#ffffff]"} value="">
                                            {field.placeholder || `Select ${field.label.toLowerCase()}`}
                                        </option>
                                        {field.options?.map((option) => (
                                            <option className={theme === 'dark' ? "bg-[#1a1a1a]" : "bg-[#ffffff]"} key={option.value} value={option.value}>
                                                {option.label}
                                            </option>
                                        ))}
                                    </select>
                                        ...
                                        (alert) =>
                                            alert && (
                                                <tr key={alert.id} className="border-b border-white/5 last:border-0">
                                                    <td className="py-3 px-4">{getTypeLabel(alert.type)}</td>
                                                    ...
                                                    <td className="py-3 px-4 text-white/60">{alert.lastTriggered || "Never"}</td>
                                                    <td className="py-3 px-4">
                                                        <button
                                    ...
                                    <div>
                                        ...
                                        <div className="grid grid-cols-2 gap-4">
                                            <select
                                                ...
                                                onChange={(e) =>
                                                    setNewAlert((prev) => ({ ...prev, condition: e.target.value as "above" | "below" }))
                                                }
                                                className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-2 outline-none focus:border-white/20"
                                            >
                                                <option className={theme === 'dark' ? "bg-[#1a1a1a]" : "bg-[#ffffff]"} value="above">
                                                    Above
                                                </option>
                                                <option className={theme === 'dark' ? "bg-[#1a1a1a]" : "bg-[#ffffff]"} value="below">
                                                    Below
                                                </option>
                                            </select>
                                            <input
                    ...
                    <div className="relative glass border border-white/10 rounded-xl p-6 w-[500px]">
                        <div className="flex items-center justify-between mb-6">
                            <div>
                                ...
                                {editAlert.type && (
                                    <div className="mt-1">{getTypeLabel(TYPE_MAPPING[editAlert.type] || editAlert.type)}</div>
                                )}
                            </div>
                            <button
                            ...
                            <div>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <select
                                        ...
                                        onChange={(e) =>
                                            setEditAlert((prev) => ({ ...prev, condition: e.target.value as "above" | "below" }))
                                        }
                                        className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-2 outline-none focus:border-white/20"
                                    >
                                        <option className={theme === 'dark' ? "bg-[#1a1a1a]" : "bg-[#ffffff]"} value="above">
