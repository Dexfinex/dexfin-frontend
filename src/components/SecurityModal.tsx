...
import { LockIcon, Loader2, Copy, CheckCircle, Trash2Icon } from "lucide-react";
...
const TwoFactorAuthInput = ({
  ...
  return (
    ...
    >
      ...
      {isDisabled && <Loader2 className="w-4 h-4 animate-spin text-blue-400" />}
    </div>
  );
};
...
export const SecuritySettings: React.FC = () => {
  ...
  const handleKeyVerify = async (code: string) => {
    setConfirming(true);
    const data = await dexfinv3Service.enable2FA(
  ...
  const handleRemove = async (code: string) => {
    setConfirming(true);
    const { success } = await dexfinv3Service.remove2FA(
      userData?.accessToken || "",
      code
    );
    if (success) {
  ...
  const fetchData = async () => {
    setLoading(true);
    const isEnabled = await dexfinv3Service.get2FAStatus(
      userData?.accessToken || ""
    );
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="bg-white/5 rounded-xl p-6 flex flex-col items-center justify-center h-64">
          ...
          <p className="text-white/60">Loading multi-factor information...</p>
        </div>
      </div>
    );
  }
            ...
            <>
              ...
              <div className="flex flex-col items-center justify-center gap-4 h-[420px]">
                ...
                {loadingSetup ? (
                  ...
                  <button
                    className="bg-blue-500 py-2 px-5 rounded-xl mb-8 hover:bg-blue-600 transition-colors"
                    onClick={handleSetup}
                  >
                    Set up now
                  </button>
          ...
          {step === 2 && (
            <>
              <div>
                ...
                <p className="text-sm text-white/60">
                  Scan the QR code or copy and paste the key into your
                  authenticator app.
                </p>
              </div>
              <div className="flex flex-col items-center justify-center gap-4 h-[420px]">
                ...
                <button
                  className="bg-blue-500 w-96 py-2 rounded-xl hover:bg-blue-600 transition-colors"
                  onClick={() => setStep(3)}
                >
                  Continue
                </button>
                <button
                  className="w-96 py-2 rounded-xl hover:bg-white/10 transition-colors"
                  onClick={() => setStep(1)}
                >
                  Cancel
                </button>
            ...
            <>
              ...
              <div className="h-[420px]">
                {confirming ? (
                  <>
                    <TwoFactorAuthInput
                      onComplete={(code: string) => handleKeyVerify(code)}
                      isDisabled={true}
                    />
                    <div className="text-center">
                      <button
                        className="mt-5 bg-white/10 py-2 px-5 rounded-xl"
                        disabled
                      >
                        Cancel
                      </button>
                  ...
                  <>
                    ...
                    <div className="text-center">
                      <button
                        className="mt-5 bg-white/10 py-2 px-5 rounded-xl hover:bg-white/5"
                        onClick={() => setStep(1)}
                      >
                        Cancel
                      </button>
            ...
            <>
              ...
              <div className="h-[420px]">
                {confirming ? (
                  <>
                    <TwoFactorAuthInput
                      onComplete={(code: string) => handleRemove(code)}
                      isDisabled={true}
                    />
                    <div className="text-center">
                      <button
                ...
                >
                  <div className="flex justify-center items-center gap-2">
                    ...
                    <div>
                      ...
                      >
                        Uses a one-time code from your authenticator app
                      </div>
                    </div>
                  </div>
