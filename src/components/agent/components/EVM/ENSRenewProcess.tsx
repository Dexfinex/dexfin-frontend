...
interface ENSRenewProcessProps {
  onClose: () => void;
  description: string;
  ensName: string;
  fromToken: TokenType;
...
export const ENSRenewProcess: React.FC<ENSRenewProcessProps> = ({ description, steps, ensName, fromToken, onClose }) => {
  ...
  const renderStep1 = () => (
    <div className="flex flex-col items-center justify-center h-full">
      ...
      <h3 className="mt-8 text-xl font-medium">Renewing ENS Name</h3>
      <p className="mt-2 text-white/60 text-center max-w-md">
        Renewing {ensName} on the Ethereum Name Service...
      </p>
    </div>
  );
  ...
  const renderStep2 = () => (
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-4 mb-6">
        <div>
          <h3 className="text-xl font-medium">ENS Name Renewed</h3>
          <p className="text-white/60">Your ENS name {ensName} has been successfully renewed.</p>
  ...
  const renderConfirmation = () => (
    <div className="flex flex-col items-center justify-center h-full text-center">
      {transactionProgress < 100 ? (
        ...
        <SuccessModal onClose={onClose} scan={scan} description={`You've successfully renewed the ENS name ${ensName}.`} />
      )}
    </div>
  );
  ...
  return (
    <div className="h-full p-6">
      ...
      {failedTransaction &&
        <FailedTransaction
          description={`Renewing ENS name ${ensName}`}
          onClose={onClose}
        />}
