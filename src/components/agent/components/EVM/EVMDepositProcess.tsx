...
export const EVMDepositProcess: React.FC<EVMDepositProcessProps> = ({ fromAmount, toToken, fromToken, protocol, onClose }) => {

  const { isLoading: isLoadingTotoken, data: toDepositToken } = useEvmTokenMetadata({ chainId: fromToken.chainId, tokenAddress: toToken.address });
...
const { address } = useContext(Web3AuthContext);
      ...
      <div className="flex-1 bg-white/5 rounded-xl p-6">
        <div className="space-y-6">
          <div className="flex flex-col md:flex-row justify-between items-center p-4 bg-white/5 rounded-lg">
            ...
            <div className="flex items-center gap-3">
              {!isLoadingTotoken &&
                <TokenChainIcon src={toDepositToken?.logo ?? ''} alt={toDepositToken?.name ?? ''}
                  size={"lg"} chainId={Number(fromToken?.chainId)} />
              }
              <div>
                <div className="text-sm text-white/60">You receive</div>
                {!isLoadingTotoken &&
                  <div className="text-xl font-medium">{formatNumberByFrac(convertCryptoAmount(fromAmount, 0))} {toDepositToken?.symbol}</div>
                }
              </div>
            </div>
          </div>
      ...
      {transactionProgress < 100 ? (
        <>
          ...
          <div className="flex items-center gap-4 animate-pulse">
            ...
            <ArrowRight className="w-6 h-6 text-white/40" />
            {toDepositToken?.logo && <img
              src={toDepositToken.logo}
              alt={toDepositToken.symbol}
              className="w-12 h-12"
            />
            }
          </div>
          <p className="mt-4 text-white/60">
            Depositing {formatNumberByFrac(convertCryptoAmount(fromAmount, 0))} {fromToken.symbol} via {protocol?.name}
          </p>
        </>
      ) : (
        <SuccessModal onClose={onClose} scan={scan} description={`You've successfully deposited ${formatNumberByFrac(convertCryptoAmount(fromAmount, 0))} ${fromToken.symbol} for ${formatNumberByFrac(convertCryptoAmount(fromAmount, 0))} ${toDepositToken?.symbol}`} />
      )}
    </div>
  );
  ...
  return (
    <div className="h-full p-6">
      ...
      {failedTransaction &&
        <FailedTransaction
          description={`Deposit ${formatNumberByFrac(convertCryptoAmount(fromAmount, 0))} ${fromToken.symbol} for ${formatNumberByFrac(convertCryptoAmount(fromAmount, 0))} ${toDepositToken?.symbol} via ${protocol?.name}`}
          onClose={onClose}
        />}
