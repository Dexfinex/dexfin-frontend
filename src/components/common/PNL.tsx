...
const PNL: React.FC<PNLProps> = ({ pnlUsd, pnlPercent, label = "" }) => {
    return (
        <div className={`flex items-center gap-1 mt-1 ${pnlUsd > 0 ? "text-green-400" : "text-red-400"}`}>
            ...
            <span className="text-xs sm:text-sm">{formatCurrency(formatNumberByFrac(pnlUsd), { symbol: '$', maximumDecimalTrailingZeroes: 2 })} {`(${formatNumberByFrac(pnlPercent || 0)}%)`} {label}</span>
        </div>
    )
}
